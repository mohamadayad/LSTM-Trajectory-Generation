...
class DataLoad():
  ...
  def munge_data(self, height = 11.0, seq_len = 20.0, dist = 3.0,verbose = True):
    ...
    - dist: the minimum distance to retina"""
    ...
    if not 'x' and 'y' and 'z' and 'timestamp' and 'rankc' in df.columns: print('We miss columns in the dataframe')

    df_arr = df.as_matrix(['x','y','z','timestamp', 'EVENTMSGTYPE', 'rankc'])
    df = None
  ...
  def entropy_offset(self):
    #calculate the self entropy for all the offsets
  ...
  def plot_needle_traj(self):
    plot_needle(self.data3,self.labels)
    return
  ...
  def plot_traj_2d(self,Nplot,extra_title=' '):
    ...
    plt.xlabel('Distance to retina (feet)')
